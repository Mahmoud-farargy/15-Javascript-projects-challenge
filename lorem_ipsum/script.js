...
const getParagraphs = (e) => {
 ...
 const result = loremList.slice(0,inputVal)?.map(el => {
    return `
      <li><p>${el}</p></li>
    `;
  }).join("");
  paragraphsList.innerHTML = result;
};

const onInputChange = (val) => {
  const currVal = val.target.value;
  inputVal = (currVal > 0 && currVal <= loremList.length +1) ? +currVal : 1;
}

// events
paragraphForm.addEventListener("submit", getParagraphs, false);
paragraphsInput.addEventListener("change", onInputChange, false);